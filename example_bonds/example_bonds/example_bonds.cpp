...
int main() {
	...
	QuantLib::Schedule schedule(effectiveDate, terminationDate, tenor, calendar, 
		convention, terminationDateConvention, rule, true, effectiveDate);

	std::vector<QuantLib::Date> days = schedule.dates(); 
	for (std::size_t sz = 0; sz < days.size(); sz++) {
		std::cout << days[sz] << ", " << days[sz].weekday() << std::endl;
	}
